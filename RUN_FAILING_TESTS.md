...
uv run python scripts/test/analyze_notebook_tests.py --failing-only --engine opensearch --batch-size 3

# Elasticsearch (10 failing notebooks)  
uv run python scripts/test/analyze_notebook_tests.py --failing-only --engine elasticsearch --batch-size 3

# Solr (4 failing notebooks)
uv run python scripts/test/analyze_notebook_tests.py --failing-only --engine solr --batch-size 2
```
...
uv run python scripts/test/analyze_notebook_tests.py --failing-only --engine opensearch --start 0 --end 3

# Run notebooks 3-5 (second batch)
uv run python scripts/test/analyze_notebook_tests.py --failing-only --engine opensearch --start 3 --end 6
```

### Run Single Notebook

```bash
uv run python scripts/test/analyze_notebook_tests.py --failing-only --engine opensearch --limit 1
```
...
- **Test Suite Documentation**: See [`tests/README.md`](tests/README.md) for comprehensive testing documentation
- **Test Scripts**: See [`scripts/README.md`](scripts/README.md) for information about other test-related scripts
