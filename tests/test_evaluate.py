...
class TestEvaluate:
    ...
    def test_evaluate_valid_mode_elastic(self):
        ...
        # Arrange
        with patch('ltr.evaluate.quiet_run') as mock_quiet, \
    ...
    def test_evaluate_copies_reports(self):
        ...
        # Arrange
        with patch('ltr.evaluate.quiet_run'), \
             patch('ltr.evaluate.log_run') as mock_log:
            # Act
            evaluate('elastic')
            # Assert
            # Check for copy commands
            copy_calls = [call[0][0] for call in mock_log.call_args_list if 'docker cp' in call[0][0]]
...
class TestRreTable:
    ...
    @patch('builtins.open', new_callable=mock_open, read_data=json.dumps({
        'metrics': {
            'P': {'versions': {'baseline': {'value': 0.5}, 'classic': {'value': 0.6}, 'latest': {'value': 0.7}}},
            'R': {'versions': {'baseline': {'value': 0.4}, 'classic': {'value': 0.5}, 'latest': {'value': 0.6}}},
            'ERR@30': {'versions': {'baseline': {'value': 0.3}, 'classic': {'value': 0.4}, 'latest': {'value': 0.5}}}
        }
    }))
    ...
    def test_rre_table_extracts_metrics(self, mock_file, mock_init, mock_iplot):
        ...
        assert header_values == ['', 'Precision', 'Recall', 'ERR'] or header_values == ('', 'Precision', 'Recall', 'ERR')
